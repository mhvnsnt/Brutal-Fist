...
export default function App() {
  ...
  useEffect(() => {
    if (screen !== AppScreen.VS || vsStartTime === null || modelLoading || modelError || !modelUrl) return;
    const timer = window.setTimeout(() => setScreen(AppScreen.Combat), Math.max(0, 2200 - (Date.now() - vsStartTime)));
    return () => window.clearTimeout(timer);
  }, [screen, vsStartTime, modelLoading, modelError, modelUrl]);
  ...
  if (screen === AppScreen.Select) return <div className="fixed inset-0 bg-black text-white flex flex-col items-center justify-center font-mono"><div className="text-xs tracking-[0.45em] text-slate-500 mb-4">CHARACTER SELECT · VERIFIED BANNON GLB ROSTER</div><div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-h-[70vh] overflow-auto p-2">{BANNON_GLB_PLAYABLE_MODELS.map((fighter, index) => <button key={`${fighter.id}:${fighter.model}`} onClick={() => { setP1Index(index); setP2Index(index === p1Index && BANNON_GLB_PLAYABLE_MODELS.length > 1 ? (index + 1) % BANNON_GLB_PLAYABLE_MODELS.length : p2Index); setScreen(AppScreen.VS); }} className="w-36 h-28 border border-white bg-[#252933] hover:bg-[#3b414d]"><div className="text-sm font-black tracking-widest">{fighter.name}</div><div className="mt-2 text-[9px] text-slate-400">{fighter.attire ?? fighter.model}</div></button>)}</div><div className="mt-4 text-[9px] tracking-[0.25em] text-slate-600">ONLY GLB_BACKED + QA-PASSED FIGHTERS ARE SELECTABLE</div></div>;
  if (screen === AppScreen.VS) return <div className="fixed inset-0 bg-black text-white flex items-center justify-center font-mono overflow-hidden"><div className="w-full px-8 flex items-center justify-between"><div className="text-3xl md:text-6xl font-black italic">{p1Name}</div><div className="text-4xl md:text-7xl font-black text-red-500">VS</div><div className="text-3xl md:text-7xl font-black italic text-slate-500">{p2Name}</div></div><div className="absolute bottom-8 left-0 right-0 text-center text-xs tracking-[0.45em] text-yellow-400 animate-pulse">{modelLoading ? 'NOW LOADING VERIFIED FIGHTER' : modelError ? 'FIGHTER ASSET FAILED — MATCH LOCKED' : 'READY'}</div>{modelError && <div className="absolute bottom-2 left-0 right-0 text-center text-[9px] text-red-400">NO FALLBACK ACTOR WILL BE USED</div>}</div>;
