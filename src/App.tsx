...
const StageSelect = dynamic(() => import('./components/StageSelect'), { ssr: false });
...
export default function App() {
  ...
  const [stageId, setStageId] = useState<Exclude<BrutalFistStageId, 'random'>>('urban-night');
  const [useNewSelect, setUseNewSelect] = useState(false);
  ...
  const p1Name = p1BannonFighter?.name ?? p1?.name;
  const p2Name = p2BannonFighter?.name ?? p2?.name;
  ...
  if (screen === AppScreen.MainMenu) return <div className="fixed inset-0 bg-[#10131a] text-white flex items-center justify-center font-mono"><div className="w-[min(86vw,420px)]"><div className="mb-10 text-xs tracking-[0.45em] text-slate-500">3D FIGHTING GAME · MOBILE</div><div className="space-y-2"><button onClick={() => { setUseNewSelect(true); setScreen(AppScreen.Select); }} className="block w-full border border-slate-600 px-6 py-4 text-left text-xl font-black tracking-widest hover:bg-white hover:text-black">ARCADE</button><button onClick={() => { setUseNewSelect(true); setScreen(AppScreen.Select); }} className="block w-full border border-slate-600 px-6 py-4 text-left text-xl font-black tracking-widest hover:bg-white hover:text-black">VERSUS</button><button onClick={() => { setUseNewSelect(true); setScreen(AppScreen.Select); }} className="block w-full border border-slate-600 px-6 py-4 text-left text-xl font-black tracking-widest hover:bg-white hover:text-black">TRAINING</button></div></div></div>;

  if (screen === AppScreen.Select) {
    if (useNewSelect) return <CharacterSelect onStartMatch={(p1f, p2f) => {
      setP1BannonFighter(p1f); setP2BannonFighter(p2f);
      const p1GlbIdx = BANNON_GLB_PLAYABLE_MODELS.findIndex(m => m.id === p1f.id);
      const p2GlbIdx = BANNON_GLB_PLAYABLE_MODELS.findIndex(m => m.id === p2f.id);
      if (p1GlbIdx >= 0) setP1Index(p1GlbIdx);
      if (p2GlbIdx >= 0) setP2Index(p2GlbIdx);
      setScreen(AppScreen.StageSelect);
    }} />;
    return <div className="fixed inset-0 bg-black text-white flex flex-col items-center justify-center font-mono"><div className="text-xs tracking-[0.45em] text-slate-500 mb-4">CHARACTER SELECT · VERIFIED BANNON GLB ROSTER</div><div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-h-[70vh] overflow-auto p-2">{BANNON_GLB_PLAYABLE_MODELS.map((fighter, index) => <button key={`${fighter.id}:${fighter.model}`} onClick={() => { setP1Index(index); setP2Index(index === p1Index && BANNON_GLB_PLAYABLE_MODELS.length > 1 ? (index + 1) % BANNON_GLB_PLAYABLE_MODELS.length : p2Index); setScreen(AppScreen.VS); }} className="w-36 h-28 border border-white bg-[#252933] hover:bg-[#3b414d]"><div className="text-sm font-black tracking-widest">{fighter.name}</div><div className="mt-2 text-[9px] text-slate-400">{fighter.attire ?? fighter.model}</div><div className="mt-2 text-[8px] text-green-400">GLB VERIFIED</div></button>)}</div></div>;
  }

  if (screen === AppScreen.StageSelect) return <StageSelect onConfirm={(selectedStage) => { setStageId(selectedStage); setScreen(AppScreen.VS); }} />;

  if (screen === AppScreen.VS) return <div className="fixed inset-0 bg-black text-white flex items-center justify-center font-mono overflow-hidden"><div className="w-full px-8 flex items-center justify-between"><div><div className="text-3xl md:text-6xl font-black italic">{p1Name}</div><div className="mt-2 text-[9px] text-slate-500">{p1?.model}</div></div><div className="text-center"><div className="text-4xl md:text-7xl font-black text-red-500">VS</div><div className="mt-2 text-[8px] tracking-[0.3em] text-white/45">{stageId.replaceAll('-', ' ').toUpperCase()}</div></div><div className="text-right"><div className="text-3xl md:text-7xl font-black italic text-slate-500">{p2Name}</div><div className="mt-2 text-[9px] text-slate-500">{p2?.model}</div></div></div><div className="absolute bottom-8 left-0 right-0 text-center text-xs tracking-[0.45em] text-yellow-400 animate-pulse">{modelLoading ? 'LOADING EXACT BANNON GLBs' : modelError ? 'FIGHTER GLB FAILED — MATCH LOCKED' : 'REAL FIGHTER ASSETS READY'}</div>{modelError && <div className="absolute bottom-2 left-0 right-0 text-center text-[9px] text-red-400">NO FALLBACK ACTOR WILL BE USED</div>}</div>;
