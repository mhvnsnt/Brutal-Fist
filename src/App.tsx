...
export default function App() {
  ...
  // Models are "ready" once loading finishes with no hard error — a null URL (404) is acceptable
  const bothModelsReady = !modelLoading && !modelError;
  ...
  if (screen === AppScreen?.MainMenu) return <div className="fixed inset-0 bg-[#10131a] text-white flex items-center justify-center font-mono"><div className="w-[min(86vw,420px)]"><div className="mb-10 text-xs tracking-[0.45em] text-slate-500">3D FIGHTING GAME</div><div className="space-y-2"><button onClick={() => { setUseNewSelect(true); setScreen(AppScreen?.Select); }} className="block w-full border border-slate-600 px-6 py-4 text-left text-xl font-black tracking-widest hover:bg-white hover:text-black">ARCADE</button><button onClick={() => { setUseNewSelect(true); setScreen(AppScreen?.Select); }} className="block w-full border border-slate-600 px-6 py-4 text-left text-xl font-black tracking-widest hover:bg-white hover:text-black">VERSUS</button><button onClick={() => { setUseNewSelect(true); setScreen(AppScreen?.Select); }} className="block w-full border border-slate-600 px-6 py-4 text-left text-xl font-black tracking-widest hover:bg-white hover:text-black">TRAINING</button></div></div></div>;
      ...
      return (
        <CharacterSelect
          onStartMatch={(p1f, p2f) => {
            ...
            // Map to GLB roster index if possible
