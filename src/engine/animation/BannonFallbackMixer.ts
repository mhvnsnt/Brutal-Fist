...
function actionCount(mixer: THREE.AnimationMixer): number {
  const internal = mixer as unknown as { _actions?: THREE.AnimationAction[] };
  return internal._actions?.length ?? 0;
}

function hasNativeActions(mixer: THREE.AnimationMixer, state: FallbackState): boolean {
  return actionCount(mixer) > state.actions.size;
}
...
async function loadFallbackActions(mixer: THREE.AnimationMixer, root: THREE.Object3D, state: FallbackState) {
  ...
  try {
    ...
    for (const [name, clip] of loaded) {
      if (!clip || state.actions.has(name)) continue;
      const action = mixer.clipAction(clip);
      action.enabled = true;
      state.actions.set(name, action);
    }
    state.ready = state.actions.size > 0;
    if (state.ready) console.log(`[BannonAnimation] fallback-ready root=${root.name || root.uuid} clips=${state.actions.size}`);
  } finally {
    state.loading = false;
  }
}
...
function selectFallbackAction(mixer: THREE.AnimationMixer, state: FallbackState, root: THREE.Object3D): void {
  if (!state.ready || hasNativeActions(mixer, state)) return;
  ...
  // This only supplies motion when Rocket's native GLB clip set is empty.
  const attacking = scale > 1.015;
  const desired = attacking ? 'COMBO_PUNCH' : speed > 0.0005 ? 'DRUNK_WALK' : 'BOX_IDLE';
  ...
  next.setEffectiveTimeScale(attacking ? 1 : speed > 0.0005 ? 1 : 0.85);
  next.setLoop(attacking ? THREE.LoopOnce : THREE.LoopRepeat, attacking ? 1 : Infinity);
  next.clampWhenFinished = attacking;
  next.play();
  if (previous && previous !== next) previous.crossFadeTo(next, attacking ? 0.035 : 0.10, false);
...
export function installBannonFallbackMixer(): void {
  if (installed) return;
  installed = true;
  const prototype = THREE.AnimationMixer.prototype as THREE.AnimationMixer & { __bfOriginalUpdate?: (delta: number) => THREE.AnimationMixer };
  ...
  prototype.update = function patchedBannonAnimationUpdate(delta: number) {
    ...
    if (!state) {
      state = { loading: false, ready: false, actions: new Map(), current: null, previousPosition: new THREE.Vector3(), initializedPosition: false };
      states.set(this, state);
    }
    if (!hasNativeActions(this, state)) {
